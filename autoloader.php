<?php
/**
 * The PSR-4 autoloader `Sixa Extras` project implementation.
 * After registering this autoload function with SPL, the following line
 * would cause the function to attempt to load the classes from different directories.
 *
 * @link       https://sixa.ch
...
spl_autoload_register(
	function( string $filename ): void { // phpcs:ignore PHPCompatibility.FunctionDeclarations.NewReturnTypeDeclarations.voidFound
		...
		if ( is_readable( $fully_qualified_path ) ) {
			require_once $fully_qualified_path; // Do not stop PHP from executing the rest of the script.
		}
	}
);
